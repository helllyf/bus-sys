...
int dePack(BYTE *ibuf,BYTE *obuf,int size)
{
  int i=1,c=0;
  BYTE r,CRCHi=0xFF,CRCLo=0xFF;
  if (ibuf[0]!=PKP_LEADER || ibuf[size-1]!=PKP_SUFFIX) return 0;
  while (i<size-1 && c<PKP_MAX_LENGTH)
  {
    switch(ibuf[i]){
      case PKP_FILLCH:
        obuf[c++]=ibuf[i+1]-1;
  ...
  if (obuf[c-1]!=CRCHi || obuf[c-2]!=CRCLo) return 0;
  else return c-2;
}
