...
static void debug_print_tuijian_xianlu ();
    ...
    while(1)
    {
        ...
        switch(cli_send_msg.cmd)
        {
            ...
            case CMD_GET_BUS_XIANLU:
            {
				...
				pthread_mutex_lock(&g_mutex);
				search_bus_xianlu (cli_send_msg.start_station, cli_send_msg.end_station);
				pthread_mutex_unlock(&g_mutex);
				printf (" CMD_GET_BUS_XIANLU \n");
				debug_print_tuijian_xianlu();
				socket_write(c_sid,&ser_bus_tuijian_msg,sizeof (ser_bus_tuijian_msg));
            }
            default:
            break;
        }
...
// 如果发送容器，需要对其序列化。这里采用转化为自定义结构体类型，再用socket发送
...
{	
	...
	{	
		...
		if (ser_bus_tuijian_msg.bus_xl_tj[way_count].end_bus_id == 
			ser_bus_tuijian_msg.bus_xl_tj[way_count].start_bus_id && i)
		{
			return ;
		}	 
	}
...
void debug_print_tuijian_xianlu ()
{	
	printf ("start from : ->%d\nturn station :%s ->%d\nend station :\n", 
